...
export const useSongsStore = defineStore("songs", () => {
  ...
  // Persist a new within-slot drag order. songIds is the desired sequence.
  async function reorderSongs(songIds: number[]): Promise<void> {
    await songsApi.reorderSongs(songIds)
    const orderMap = new Map<number, number>()
    songIds.forEach((id, i) => orderMap.set(id, i))
    dormSongs.value = dormSongs.value.map((s) =>
      orderMap.has(s.id) ? { ...s, order: orderMap.get(s.id) } : s
    )
  }

  return { dormSongs, broadcastSongs, loading, fetchSongs, addSong, updateSong, deleteSong, assignSong, unassignSong, reorderSongs }
})
